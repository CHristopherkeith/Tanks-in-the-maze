...
var Spy=function(){
	...
	this.firerate=15;//15射击速率，适当设置避免子弹过快或者过慢绘制
	...
	this.fire=function(){	
		switch(this.patharr[this.cnt]||this.patharr[this.patharr.length-1]){
			case "up":
				this.bulletLibrary["up"].get(this.x+15,this.y-10,3);//3
				break;
			case "down":
		...
		for(var i=0;i<this.enemy.length;i++){
			if(this.enemy[i].counter>this.enemy[i].firerate&&this.enemy[i].spyInRange&&!this.enemy[i].isColliding){
				...
				case "left":
					...
					break;
				case "right":
					this.enemyBulletLibrary["right"].get(this.enemy[i].x+28,this.enemy[i].y+15,.3);
					break;
				}
